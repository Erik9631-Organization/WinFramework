...
void ConcurrentShapeRenderer::DrawFillRectangle(const glm::vec3 &pos, const glm::vec3 &size)
{
    ...
    for(int y = 0; y < rectangle.size.y; y++)
    {
        ...
        auto* p = buffer.GetOffset(offsetY + rectangle.position.x);
        for(int width = rectangle.size.x; width != 0; width--)
        {
            *p = color;
            p++;
        }
...
{
    this->color = bufferRenderer->CreateColor(color);
}

void ConcurrentShapeRenderer::SetColor(const glm::ivec3 &color)
{
    this->color = bufferRenderer->CreateColor({color, 255});
}
...
ConcurrentShapeRenderer::ConcurrentShapeRenderer()
{
    numberOfThreads = 1;
}

void ConcurrentShapeRenderer::DrawEllipse(const glm::vec3 &pos, const glm::vec3 &size, bool drawFromCenter)
{
    ZoneScoped;
//    int height = size.y;
...
ConcurrentShapeRenderer::SplitRectangle(const Rectangle &rectangle) const
{
    ...
    {
        auto rect = IRectangle{rectangle.position, rectangle.size};
...
void ConcurrentShapeRenderer::DrawFillEllipse(const glm::vec3 &pos, const glm::vec3 &size, bool drawFromCenter)
{
    ZoneScoped;
//    int height = size.y;
...
{
    ZoneScoped;
    if(rectangle.position.y < 0 || rectangle.position.x < 0) // If outside screeen bounds
        return true;

    if( (rectangle.position.y + rectangle.size.y) > buffer.GetHeight() || (rectangle.position.x + rectangle.size.x) > buffer.GetWidth()) // If outside screeen bounds
        return true;
...
void ConcurrentShapeRenderer::SetScreenBuffer(BufferRenderer &buffer)
{
    bufferRenderer = &buffer;
    this->buffer = buffer.GetScreenBuffer();
}
