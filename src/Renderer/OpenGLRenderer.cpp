...
void OpenGLRenderer::GetGlExtensions()
{
...
void OpenGLRenderer::AssignRendererToNodes()
{
...
void OpenGLRenderer::AssignGraphicsToNodes(MultiTree<unique_ptr<UiElement>> &node)
{
    if(!node.IsRoot())
        renderingPool->SetTranslation(node.GetValue()->GetPosition());
    else
        renderingPool->SetTranslation({0, 0, 0});

    RenderEventInfo renderEvent{renderingPool.get(), this};
...
void OpenGLRenderer::GraphicsInit()
{
